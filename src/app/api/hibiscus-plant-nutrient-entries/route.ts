...
const nutrientEntriesFile = path.join(dataDir, 'hibiscus-plant-nutrient-entries.json');
...
const loadEntries = async (): Promise<NutrientEntry[]> => {
  try {
    ...
    console.error('Failed to load hibiscus plant nutrient entries:', error);
  }
  return [];
};
...
const saveEntries = async (entries: NutrientEntry[]): Promise<void> => {
  try {
    ...
    console.error('Failed to save hibiscus plant nutrient entries:', error);
    throw error;
  }
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Hibiscus plant nutrient entries fetch error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch hibiscus plant nutrient entries' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Hibiscus plant nutrient entry save error:', error);
    return NextResponse.json(
      { error: 'Failed to save hibiscus plant nutrient entry' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Hibiscus plant nutrient entry delete error:', error);
    return NextResponse.json(
      { error: 'Failed to delete hibiscus plant nutrient entry' },
      { status: 500 }
    );
  }
}