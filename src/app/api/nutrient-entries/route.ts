...
const nutrientEntriesFile = path.join(dataDir, 'nutrient-entries.json');
...
const saveEntries = async (entries: NutrientEntry[]): Promise<void> => {
  try {
    ...
    console.error('Failed to save nutrient entries:', error);
    throw error;
  }
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Nutrient entries fetch error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch nutrient entries' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Nutrient entry save error:', error);
    return NextResponse.json(
      { error: 'Failed to save nutrient entry' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Nutrient entry delete error:', error);
    return NextResponse.json(
      { error: 'Failed to delete nutrient entry' },
      { status: 500 }
    );
  }
}