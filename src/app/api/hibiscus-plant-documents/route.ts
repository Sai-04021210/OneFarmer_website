...
const uploadDir = path.join(process.cwd(), 'public/uploads/hibiscus-plant-documents');
...
const loadMetadata = async (): Promise<DocumentMetadata[]> => {
  try {
    ...
    console.error('Failed to load hibiscus plant document metadata:', error);
  }
  return [];
};
...
const saveMetadata = async (metadata: DocumentMetadata[]): Promise<void> => {
  try {
    ...
    console.error('Failed to save hibiscus plant document metadata:', error);
  }
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use HIBISCUS_PLANT as stemId for hibiscus plant document uploads
    ...
    return NextResponse.json({
      ...
      document: {
        ...documentMetadata,
        url: `/uploads/hibiscus-plant-documents/${filename}`,
      },
    });

  } catch (error) {
    console.error('Hibiscus plant document upload error:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const documentsWithUrls = documents.map(doc => ({
      ...doc,
      url: `/uploads/hibiscus-plant-documents/${doc.filename}`,
    }));

    return NextResponse.json({ documents: documentsWithUrls });

  } catch (error) {
    console.error('Hibiscus plant document fetch error:', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Hibiscus plant document delete error:', error);
    return NextResponse.json(
