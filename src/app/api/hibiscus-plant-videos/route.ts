...
const uploadDir = path.join(process.cwd(), 'public/uploads/hibiscus-plant-videos');
...
const loadMetadata = async (): Promise<VideoMetadata[]> => {
  try {
    ...
    console.error('Failed to load hibiscus plant video metadata:', error);
  }
  return [];
};
...
const saveMetadata = async (metadata: VideoMetadata[]): Promise<void> => {
  try {
    ...
    console.error('Failed to save hibiscus plant video metadata:', error);
  }
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Use HIBISCUS_PLANT as stemId for hibiscus plant video uploads
    ...
    return NextResponse.json({
      ...
      video: {
        ...videoMetadata,
        url: `/uploads/hibiscus-plant-videos/${filename}`,
      },
    });

  } catch (error) {
    console.error('Hibiscus plant video upload error:', error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const videosWithUrls = videos.map(video => ({
      ...video,
      url: `/uploads/hibiscus-plant-videos/${video.filename}`,
    }));

    return NextResponse.json({ videos: videosWithUrls });

  } catch (error) {
    console.error('Hibiscus plant video fetch error:', error);
    return NextResponse.json(
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.error('Hibiscus plant video delete error:', error);
    return NextResponse.json(
