...
export default function RosePlantPage() {
  ...
  const [historicalData, setHistoricalData] = useState<PlantHistoricalEntry[]>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('rosePlantHistoricalData');
  ...
  const [autoMode, setAutoMode] = useState<Record<string, boolean>>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('rosePlantAutoMode');
      return saved ? JSON.parse(saved) : {
        stemA_height: true,
  ...
  const [headerPhotos, setHeaderPhotos] = useState<{[key: string]: string}>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('rosePlantHeaderPhotos');
      if (saved) {
        try {
  ...
  const [stemPhotos, setStemPhotos] = useState<{[key: string]: string[]}>(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('rosePlantStemPhotos');
      if (saved) {
        try {
  ...
  const [hydroManualMode, setHydroManualMode] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('roseHydroponicManualMode');
      return saved ? JSON.parse(saved) : {
        ph: false,
        ec: false,
        waterTemp: false,
  ...
  const [hydroManualValues, setHydroManualValues] = useState(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('roseHydroponicManualValues');
      return saved ? JSON.parse(saved) : {
        ph: 0,
        ec: 0,
        waterTemp: 0,
  ...
  useEffect(() => {
    const loadPlantParameters = async () => {
      try {
        const response = await fetch('/api/rose-plant-parameters');
        if (response.ok) {
    ...
    const loadStemHistory = async () => {
      ...
      for (const stem of ['A', 'B', 'C', 'D']) {
        try {
          const response = await fetch(`/api/rose-plant-stem-history?stemId=${stem}`);
          if (response.ok) {
            const data = await response.json();
            stemData[stem] = data.history || [];
          } else {
            stemData[stem] = [];
          }
        } catch (error) {
    ...
    const loadHydroponicData = async () => {
      try {
        const response = await fetch('/api/rose-hydroponic-entries');
        if (response.ok) {
          const data = await response.json();
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('roseHydroponicManualMode', JSON.stringify(hydroManualMode));
    }
  }, [hydroManualMode]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('roseHydroponicManualValues', JSON.stringify(hydroManualValues));
    }
  }, [hydroManualValues]);
  ...
  useEffect(() => {
    const savePlantParameters = async () => {
      try {
        await fetch('/api/rose-plant-parameters', {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('rosePlantHistoricalData', JSON.stringify(historicalData));
    }
  }, [historicalData]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('rosePlantAutoMode', JSON.stringify(autoMode));
    }
  }, [autoMode]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('rosePlantHistoricData', JSON.stringify(plantHistoricData));
    }
  }, [plantHistoricData]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        ...
        localStorage.setItem('rosePlantStemPhotos', JSON.stringify(limitedStemPhotos));
      } catch (error) {
        ...
        if (error instanceof DOMException && error.name === 'QuotaExceededError') {
          ...
          try {
            localStorage.setItem('rosePlantStemPhotos', JSON.stringify(veryLimitedStemPhotos));
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      try {
        localStorage.setItem('rosePlantHeaderPhotos', JSON.stringify(headerPhotos));
      } catch (error) {
        console.warn('Failed to save header photos to localStorage:', error);
        if (error instanceof DOMException && error.name === 'QuotaExceededError') {
          console.log('Clearing localStorage to free up space...');
          localStorage.removeItem('rosePlantHeaderPhotos');
        }
      }
    }
  }, [headerPhotos]);
  ...
  const saveStemData = async (stem: string) => {
    ...
    try {
      const response = await fetch('/api/rose-plant-stem-history', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        const historyResponse = await fetch(`/api/rose-plant-stem-history?stemId=${stem}`);
  ...
  const exportHydroponicDataAsCSV = () => {
    ...
    link.setAttribute('download', `rose_hydroponic_data_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const exportHydroponicDataAsJSON = () => {
    ...
    const exportData = {
      ...
      description: 'Hydroponic system water parameters monitoring data for Rose plant',
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `rose_hydroponic_data_${new Date().toISOString().split('T')[0]}.json`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportPlantDataAsCSV = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `rose_plant_aggregated_data_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const exportCompleteDataAsCSV = () => {
    ...
    try {
      ...
      // 1. Collect Rose Plant Data from state variables
      ['A', 'B', 'C', 'D'].forEach(stem => {
        if (typeof window !== 'undefined') {
          ...
          if (stemHistory) {
            ...
            stemData.forEach((entry: any) => {
              ...
              allData.push({
                ...
                dataType: 'Rose Plant - Individual Stems',
                date: entryDate,
                time: entryTime,
                stem: `Stem ${stem}`,
      ...
      historicalData.forEach(entry => {
        allData.push({
          category: 'Plant Growth',
          dataType: 'Rose Plant - Overall Health',
    ...
    try {
      ...
      ['A', 'B', 'C', 'D'].forEach(stem => {
        if (typeof window !== 'undefined') {
          ...
          if (stemHistory) {
            try {
              exportData.rosePlantData.stems[`stem${stem}`] = JSON.parse(stemHistory);
            } catch (e) {
      ...
      let totalRecords = 0;
      totalRecords += Object.values(exportData.rosePlantData.stems).reduce((sum, stemData) => sum + stemData.length, 0);
      totalRecords += exportData.rosePlantData.historicalData.length;
      totalRecords += exportData.rosePlantData.plantHistoricData.length;
  ...
  const fetchUploadedPhotos = useCallback(async () => {
    try {
      const response = await fetch('/api/plant-photos?page=rose-plant');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleEnhancedPhotoUpload = async (files: FileList, metadata: {
    ...
    for (const file of fileArray) {
      try {
        ...
        formData.append('page', 'rose-plant');
  ...
  const handleEnhancedVideoUpload = async (files: FileList, metadata: {
    ...
    for (const file of fileArray) {
      try {
        ...
        const response = await fetch('/api/rose-plant-videos', {
          method: 'POST',
          body: formData,
        });
  ...
  const fetchUploadedVideos = useCallback(async () => {
    try {
      console.log('Fetching videos from /api/rose-plant-videos...');
      const response = await fetch('/api/rose-plant-videos?page=rose-plant');
  ...
  //   try {
  //     const response = await fetch(`/api/rose-plant-videos?videoId=${videoId}`, {
  //       method: 'DELETE',
  //     });
  ...
  const fetchUploadedDocuments = useCallback(async () => {
    try {
      console.log('Fetching documents from /api/rose-plant-documents...');
      const response = await fetch('/api/rose-plant-documents?page=rose-plant');
  ...
  const fetchUploadedLinks = useCallback(async () => {
    try {
      console.log('Fetching links from /api/rose-plant-links...');
      const response = await fetch('/api/rose-plant-links?page=rose-plant');
  ...
  const downloadUploadedDocument = (document: any) => {
    const link = document.createElement('a');
    link.href = `/uploads/rose-plant-documents/${document.filename}`;
  ...
  //   try {
  //     const response = await fetch(`/api/rose-plant-documents?documentId=${documentId}`, {
  //       method: 'DELETE',
  //     });
  ...
  const handleEnhancedDocumentUpload = async (files: FileList, metadata: {
    ...
    for (const file of fileArray) {
      try {
        ...
        const response = await fetch('/api/rose-plant-documents', {
          method: 'POST',
          body: formData,
        });
  ...
  const deleteLink = async (linkId: string) => {
    ...
    try {
      const response = await fetch(`/api/rose-plant-links?linkId=${linkId}&page=rose-plant`, {
        method: 'DELETE',
      });
  ...
  const handleEnhancedLinkUpload = async (metadata: {
    ...
    try {
      ...
      formData.append('stemId', metadata.stemId || 'ROSE_PLANT');
      formData.append('category', metadata.category);
      formData.append('page', 'rose-plant');
      if (metadata.description) formData.append('description', metadata.description);
      if (metadata.tags) formData.append('tags', metadata.tags);
      ...
      const response = await fetch('/api/rose-plant-links', {
        method: 'POST',
        body: formData,
      });
  ...
  const exportPlantDataAsJSON = () => {
    ...
    const exportData = {
      metadata: {
        exportDate: new Date().toISOString(),
        dataType: 'Rose Plant Growth Monitoring Data',
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `rose_plant_complete_data_${new Date().toISOString().split('T')[0]}.json`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
              {entry.type === 'Full Plant Data' ? (
                ...
                <>
                  ...
                  {(entry.data as Record<string, unknown>)?.diseases && (
                    <div className="col-span-2 md:col-span-4"><span className="font-medium">Issues:</span> {String((entry.data as Record<string, unknown>)?.diseases)}</div>
                  )}
                </>
              )}
            </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
            <h1 className={`text-2xl sm:text-3xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Rose Plant Management</h1>
          </div>
        ...
        <div className="mb-12">
          <div className={`rounded-xl p-4 sm:p-6 lg:p-8 border transition-colors duration-300 ${
            ...
            <div className="flex items-center mb-6">
              ...
              <h4 className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Hydroponic Parameters - Rose Plant</h4>
            </div>
            <div className="text-center mb-6">
              <p className={`${
                isDarkMode ? 'text-blue-400' : 'text-blue-600'
              }`}>Currently monitoring Dutch Bucket system for Rose plant cultivation</p>
            </div>
        ...
        <div className={`rounded-xl p-6 border shadow-sm transition-colors duration-300 ${
          ...
          <div className={`rounded-xl p-4 sm:p-6 lg:p-8 border transition-colors duration-300 ${
            ...
            <NutrientTracker isDarkMode={isDarkMode} plantType="rose" />
          </div>
        </div>
            ...
            <div className="p-4">
              ...
              <div className="flex justify-center mb-4">
                {selectedDocument.originalName?.toLowerCase().endsWith('.pdf') ? (
                  <div className="w-full">
                    <iframe
                      src={`/uploads/rose-plant-documents/${selectedDocument.filename}#toolbar=1&navpanes=1&scrollbar=1`}
                    ...
                    <div className="mt-2 text-center">
                      <a
                        href={`/uploads/rose-plant-documents/${selectedDocument.filename}`}
                        target="_blank"
